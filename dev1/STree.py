# a = 'SE|s57375967.txt||tx|1|text|1|91|Focal consolidation at the left lung base, possibly representing aspiration or  pneumonia.\nSE|s57375967.txt||tx|1|entity|C0205234|Focal|spco|||Focal|||0|888|1|6\nSE|s57375967.txt||tx|1|entity|C0702116|Consolidation|qlco|||consolidation|||0|888|7|20\nSE|s57375967.txt||tx|1|entity|C0225730|Left lung|bpoc|||left lung|||0|901|28|37\nSE|s57375967.txt||tx|1|entity|C0002055|Alkalies|inch|||base|||0|901|38|42\nSE|s57375967.txt||tx|1|entity|C2362652|Possible diagnosis|qlco|||possibly|||0|1000|44|52\nSE|s57375967.txt||tx|1|entity|C0349707|Aspiration-action|topp|||aspiration|||0|1000|66|76\nSE|s57375967.txt||tx|1|entity|C0032285|Pneumonia|dsyn|||pneumonia|||0|1000|81|90'
...
class STree():
    ...
    def find_mrepeats(self, m, k):
        """ m: min repeat times; k: min length of a pattern """
        result = sorted(self._find_mr_traverse(self.root, m, k), key=lambda x: x[2])

        """ remove all substring within another patter """
        leftstart = 0
        leftend = 0
        final = []
        for pattern in result:
            if len(pattern[0]) + pattern[2] > leftend:
                final = final + [pattern]
                leftstart = pattern[2]
                leftend = len(pattern[0]) + pattern[2]

        return final
        #return self._find_mr_traverse(self.root, m, k)

    def _find_mr_traverse(self, n, m, k):
        """ Helper method, return a list of the longest m-repeats  """
        repeat = 0
        pattern = []
        edge = self._edgeLabel(n, self.root)
        if len(edge) > k and not edge[0].isspace(): # must start by a non-space
            for child, _ in n.transition_links:
                repeat = repeat + len(child._get_leaves())
            if repeat < m:
                return []
            pattern = [[edge, repeat, n.idx]]
        """ continue traversing """
        result = []
        for (node,_) in n.transition_links:
            result = result + self._find_mr_traverse(node, m, k)

        if pattern != [] and pattern[0][2] in list(map(lambda x: x[2], result)):
        # if a longer pattern is found to repeat more than m times then discard the shorter 
            return result
        else:
